...
def toggle_faces_color(path):
    # manual differenciation if this face belongs to the classified class (fastener) or not
    selected = "red"
    uncolored = "yellow"
    colored = "blue"
    ...
    for i in range(num_faces):
        color_nth_advanced_face("../data/Control_levers_n.stp", i, uncolored)
    ...
    for i in range(num_faces):      #toggle manually selecting face
        def on_release(key):
            try:
                ...
                if key.char == 'y':
                    # select current face
                    print("y pressed. selecting surnface " + str(i))
                    color_nth_advanced_face("../data/Control_levers_n.stp", i, colored)
                    # Stop listener
                    return False
            except:
                pass  # special key pressed
        color_nth_advanced_face("../data/Control_levers_n.stp", i, selected)
